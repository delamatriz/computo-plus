...
> **Nota para el diseño de esta UI (agregada durante una corrida de prueba de la etapa 3, ago-2026):** el caso `MAT-BIDET-BLANCO` mostró que el modelo puede encontrar y comparar un producto de categoría distinta al original (un "Kit de inodoro" en vez de un "Bidet") sin marcarlo explícitamente como "producto distinto" — solo quedó atajado porque la variación de precio resultante (155,7%) superó el umbral por casualidad. Un caso similar con una variación menor podría colarse sin que nadie lo note. La UI de revisión NO debe mostrar solo el % de variación como si fuera la única señal de alerta — tiene que pedirle a quien revisa que confirme explícitamente que el producto encontrado (nombre/ficha) es el mismo que el original antes de aprobar el cambio, no solo el precio.
